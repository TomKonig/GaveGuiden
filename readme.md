...
### 3.2 Data Flow and Component Interaction

Our architecture is built on a strategic separation of concerns, designed to maximize client-side performance while leveraging serverless functions for secure, specific, and scalable backend operations. The following details the lifecycle of a user session, from initial page load to the final gift recommendation, illustrating how our core components—the **Client** (user's browser), **CDN** (Netlify), **Serverless Functions** (Netlify Functions), and **Database** (MongoDB Atlas)—work in concert.

#### 1. Application Initialization & Static Asset Loading

The user's journey begins with a single request to <https://www.denrettegave.dk>. This request is intercepted by **Netlify's global CDN**, which immediately serves the core application shell from a server geographically close to the user, ensuring the fastest possible initial load time. The assets served are entirely static and include:

- index.html: The main document structure.
- style.css: The visual styling and layout rules.
- quiz-logic.js: The primary JavaScript file containing all the client-side quiz logic, scoring algorithms, and state management.

This initial exchange is heavily optimized for speed, delivering a near-instant "time to first paint" and ensuring the user can begin interacting with the quiz without perceptible delay.

#### 2. Initial State: Loading the Full Product & Interest Catalogues

Immediately after the core application shell loads, quiz-logic.js initiates two critical background requests to fetch our data catalogs, which are also hosted as static files on the CDN:

- assets/products.json: A complete JSON array of every product in our catalog, containing details like id, name, price, url, and an array of descriptive tags.
- assets/interests.json: A hierarchical JSON object that defines the relationships between parent and child interests (e.g., "Sport" is a parent of "Fodbold").

This "pre-caching" of the entire dataset is a cornerstone of our architecture. By loading all products and interests into the browser's memory at the start, we empower the client to perform all subsequent filtering and scoring operations locally. This eliminates the latency of traditional database queries during the quiz, providing an exceptionally fluid user experience. These files are also subject to browser caching, meaning returning visitors will often have them stored locally, further speeding up the initialization process.

#### 3. Interaction 1: Applying Hard Filters (Gender, Budget, Age)

The first interactive phase of the quiz involves "hard filters." These are non-negotiable criteria that a product must meet to remain under consideration. As the user makes their selections for **Gender**, **Budget**, and **Age**, the quiz-logic.js script performs these filtering operations in real-time.

For each selection, the script iterates through the in-memory array of products and applies the corresponding filter:

- **Gender:** If the user selects "Mand" (Man), any product tagged exclusively for women (e.g., gender:woman_exclusive) is removed from the active product pool. Products tagged gender:all or gender:man are retained. This is a purely negative filter designed to exclude irrelevant items without positively biasing gender-specific products over unisex ones.
- **Budget:** The user's selection maps to a min and max price range on the backend. Any product whose price falls outside this range is discarded. Critically, a user in a "high" budget category is never shown products from the "low" budget category unless they explicitly opt-in, but a user in a "low" budget category will _never_ see products from a higher price tier.
- **Age:** Certain products, like those intended for children, are hard-filtered out if the selected age range is for an adult, and vice-versa.

Each of these filtering steps progressively shrinks the array of product objects being actively considered, all without a single call to a server.

#### 4. Interaction 2: Applying Soft Filters & Initial Scoring (Relation, Occasion)

Once hard filters have narrowed the field, the quiz begins applying "soft filters." Unlike hard filters, these do not eliminate products. Instead, they influence a product's relevance by contributing to a score. When the user selects their **Relation** to the recipient (e.g., _Ven_ - Friend) and the **Occasion** (e.g., _Fødselsdag_ - Birthday), quiz-logic.js initializes a scoring object in memory.

This object maps every product.id remaining in the pool to an initial score of 0. The script then iterates through the product pool and inspects each product's tags array. If a product has a tag that matches the user's selection (e.g., relation:friend or occasion:birthday), its score is incremented by a predetermined amount. This process establishes a baseline ranking, giving an initial advantage to products that are generally appropriate for the social context of the gift, all while remaining entirely within the client's browser.

#### 5. Interaction 3: Hierarchical Scoring via the Interest Module

This is the most critical phase of client-side scoring, where the system deciphers the recipient's specific tastes. The user interacts with our "interests hub," either by selecting suggested interest "pills" or by searching directly for a topic. The scoring logic here is sophisticated and relies heavily on the hierarchy defined in interests.json.

- **Weighted Scoring:** When an interest is selected, quiz-logic.js awards points to products with matching tags. Crucially, the more granular the interest, the higher the score awarded. A product matching a specific child interest like "Golf" will receive significantly more points than a product matching only its broad parent category, "Sport." This rewards user specificity and helps to quickly differentiate between products.
- **Hierarchical Logic:** The system understands parent-child relationships. If a user selects "Sport," products tagged with "Sport" or any of its descendants (like "Fodbold" or "Basketball") will receive points. Conversely, if a user searches for and selects a specific interest like "Paddleboarding," the script automatically applies points for that tag and also traverses _up_ the tree in interests.json to apply points for its parents ("Vandsport," "Sport," "Oplevelser"), ensuring that a specific choice still reinforces broader categories. This creates a rich, interconnected scoring web that accurately reflects the nuances of the user's input.

#### 6. The "Race": Continuous Score Evaluation on the Client

The scoring process is not a single calculation but a dynamic "race" that is re-evaluated after every user input. With each filter applied or interest selected, the scores for all products in the pool are updated. Immediately following each update, a checkWinner() function runs within quiz-logic.js.

A product is declared the "winner" and the race concludes only when two conditions are met simultaneously:

1. **Absolute Threshold:** The leading product's score must exceed a predefined minimum value. This ensures the gift is a genuinely good match, not just the "least bad" option.
2. **Relative Threshold:** The leading product's score must be greater than the second-place product's score by a significant percentage. This prevents a premature result in cases where several products are very closely matched and more information is needed to make a confident recommendation.

This continuous evaluation loop ensures that as soon as a sufficiently confident match is found, the result is presented to the user immediately. If the user completes the initial questions and no product has met both winning conditions, the quiz progresses to the next phase: AI-enhanced disambiguation.

#### 7. The Handoff: Triggering the AI Disambiguation Logic

This critical transition occurs if the client-side "race" concludes without a clear winner. Having exhausted the initial demographic and interest data, quiz-logic.js determines that it cannot make a confident recommendation. At this point, it prepares for the first and only handoff to our backend for intelligent assistance.

A lean, context-rich payload is assembled on the client. This payload does not contain the entire state of the quiz, but only the essential information required for the next step:

- A list of the product ids for the leading products still in contention.
- A consolidated and de-duplicated list of all tags associated with those specific contenders.

This carefully constructed object is then sent via a secure POST request to our serverless function endpoint: /.netlify/functions/generate-ai-question. This is the first time any data about the user's specific quiz session leaves the browser.

#### 8. Serverless Execution: Secure AI Prompt Generation

The generate-ai-question.js function, executing in Netlify's secure serverless environment, receives the payload from the client. This function has two primary responsibilities: to act as a secure proxy and as an expert, multi-layered prompt engineer.

1. **Secure Proxy:** The function retrieves our secret OpenAI API key from a protected environment variable. This ensures the key is never exposed to the client-side browser, completely safeguarding it from public access.
2. **Multi-Layered Prompt Engineering:** The function's core task is to dynamically construct a highly-specific prompt for the AI model. The prompt instructs the AI to follow a prioritized strategy to determine the most effective question to ask:
    - **Priority 1: Find the Direct Differentiator.** The AI first checks if the top contenders are simple variants of each other (e.g., differentiated only by a variation tag like variation:teak_wood). If so, it is instructed to return a direct, simple question to resolve this, as this is the most efficient path to a final answer.
    - **Priority 2: Probe Niche Interest Outliers.** If no simple differentiator is found, the AI's next task is more complex. It analyzes the user's scoring pattern across all interest tags. The prompt instructs it to look for statistical outliers—specifically, to identify if the user has shown a disproportionately high affinity for a **niche category** (one with few products) that wouldn't normally rank near the top. If the user's score for "Fluefiskeri" (Fly-fishing) is significantly higher than the average or mathematically expected, the AI is instructed to formulate a "probe" question about this niche, even if "Køkkenudstyr" (Kitchen Equipment) is the dominant category by total score. This is our key mechanism for ensuring that high-relevance niche products are not drowned out by larger categories.
    - **Priority 3: Break the General Deadlock.** If neither of the above conditions is met, the AI falls back to its general disambiguation task: analyzing the top-scoring categories and formulating a broader question that can best differentiate between them to break the current deadlock.

The function then sends this purpose-built prompt to the OpenAI API and awaits a response.

#### 9. The Return: Parsing the AI Response on the Client

Once the OpenAI API processes the request, it returns a structured JSON object containing the generated question and corresponding answers to our serverless function. The function immediately relays this JSON object back to the client's browser as the response to the original fetch request.

quiz-logic.js receives this payload and parses it. It then dynamically injects the new, intelligent question and its answer choices into the quiz interface for the user. Crucially, the script also maps each answer choice back to the specific tag or tags it represents (e.g., the answer "An outdoor experience" is mapped to the category:experiences tag). This mapping is held in memory, ready to be used in the final scoring calculation once the user makes their selection. From the user's perspective, this entire roundtrip is seamless; a new, relevant question simply appears, guiding them toward the final result.

#### 10. Final Scoring, Variant Takeover, & Determining the Winning Product

This final step is a multi-stage process that occurs after the user has answered an AI-generated question. The system's goal is to definitively select a single product, either directly from the AI's input or by taking over for a final, hard-coded choice.

1. **Post-AI Scoring:** quiz-logic.js first applies a substantial score increase to the product(s) matching the user's answer to the AI's question. This significantly narrows the field, but may not necessarily conclude the race.
2. **Variant Check:** After the scores are updated, the script performs a crucial check. It analyzes the few remaining top contenders to see if they are essentially identical products, differentiated only by a variation tag (e.g., variation:teak_wood vs. variation:stainless_steel).
3. **The "Takeover" (Conditional):** If the system detects this specific scenario, it **takes over** from the AI. Instead of declaring a winner, quiz-logic.js triggers a hard-coded, simple question based on the detected variation tags. For example, the user might be asked, "Which material would they prefer: Teak Wood or Stainless Steel?". This avoids spending another costly AI API call on a simple, binary choice that can be handled more efficiently by our own logic.
4. **Final Selection:**
    - If the "Takeover" was triggered, the user's answer to the hard-coded variant question provides the final, decisive input. The selected product is declared the winner.
    - If the "Variant Check" was negative (meaning the top contenders were not simple variants of each other), the product with the highest score after the Post-AI Scoring is declared the winner.

In either path, the race concludes here, and the script transitions to the results view to present the final, winning recommendation.

#### 11. Ancillary Interaction: Creating a Shareable Result Link

This interaction is user-initiated and occurs outside the core quiz flow, on the final results page. Should the user wish to save or share their gift recommendation, they can click the "Share" button.

1. **Client Action:** The browser packages the product.id of the winning gift, along with the IDs of the top runners-up, into a JSON payload.
2. **API Call:** It sends this payload via a POST request to our /.netlify/functions/create-share-link serverless function.
3. **Serverless & Database Execution:** The function receives the list of product IDs. It generates a unique identifier for this specific result set. Using our mongodb-client.js utility, it connects to our MongoDB Atlas database and inserts a new document into the shares collection, containing the new unique ID and the associated product IDs.
4. **Response:** The function returns the unique ID to the client, which then constructs the full shareable URL (e.g., <https://denrettegave.dk/share/{uniqueId}>) and displays it to the user.

#### 12. Ancillary Interaction: Submitting Product Ratings & Flags

To maintain a high-quality product catalog and user experience, we rely on user feedback. From the results page, a user can rate a recommendation or flag it for review.

1. **Client Action:** The user interacts with the feedback UI. The browser assembles a payload containing the product.id, the rating (e.g., a score from 1-5), or the reason for the flag.
2. **API Call:** This payload is sent via a POST request to the appropriate backend endpoint: submit-rating.js or submit-flag.js.
3. **Serverless & Database Execution:** The serverless function receives the data, performs validation, and writes a new entry into the corresponding ratings or flags collection in our MongoDB Atlas database.

This provides a direct feedback loop, allowing our content curation team to review poorly-rated products or address issues (like broken links or incorrect categorization) that have been flagged by the community.

#### 13. Fault Tolerance & Graceful Degradation

Our system is architected to be resilient. Because the core filtering and scoring logic runs entirely on the client, the quiz remains functional even if our backend services are temporarily unavailable. However, for interactions that require a serverless function, we have implemented clear fault tolerance.

Every fetch call made from the client to a serverless function is wrapped in try...catch blocks. If a function fails to respond (due to a timeout, a server-side error, or the OpenAI API being down), the catch block prevents the application from crashing. Instead of proceeding with the AI-powered logic, the system will gracefully degrade. In this scenario, it will bypass the AI question and instead present the user with the current top-ranked gift, even if it hasn't met the full winning threshold. While not the ideal outcome, this ensures the user is never met with a dead end or a broken page, preserving the user experience above all else.
