...
2.  **Conditional Path Mandate:** You must use the provided product 'Context' data to respect our hard filters (\`gender\` and \`budget\`). If these filters create a significantly different set of products, you **must generate separate, conditional question paths**. Each question object must include a \`context\` field specifying the filter permutation it applies to (e.g., { "context": { "gender": "man" } }). If a question is generic, the context can be null.
3.  **Context Keys:** You MUST NOT invent new context keys or tags relating to these. Do not create "budget" tags, for example. Simply use the context keys to determine the number of questions necessary to account for all combinations of users. Importantly, a man will only be recommended products with the gender context keys "man" or "all". A user with a "billig" budget will only be recommended products with the "billig" budget key, whereas someone with a "dyr" budget will be recommended products from all three categories. It is a hard ceiling, but not a hard floor. You must account for this. Age has only one strict limitation: We do not recommend strictly childrens' products to adults.
