...
function getScopedInterestTree(allInterests, parentKey) {
    ...
    const findChildrenRecursive = (key) => {
        if (relevantInterests.has(key)) return;
...
You will be given a top-level category, a list of products ONLY for that category, the full hierarchical tree of interests for context, and strategic feedback from previous weeks. Your task is to generate a questions.json structure for this single category.
...
2.  **Conditional Path Mandate:** You must also consider the core user filters (age, gender, budget), paying special attention to the **hard filters**: \`gender\` and \`budget\`. If these filters create a significantly different set of products for a given category, you **must generate separate, conditional question paths** for that context. Each question object must include a \`context\` field specifying the filter permutation it applies to (e.g., { "context": { "gender": "man" } }). If a question is generic, the context can be null.
3.  **Pronoun Templating Mandate:** When writing question text, if you need to use a gendered pronoun, you MUST use the following placeholders: \`{{pronoun1}}\` (han/hun/de), \`{{pronoun2}}\` (ham/hende/dem), and \`{{pronoun3}}\` (hans/hendes/deres).
4.  **Semantic Variations:** For each logical question you create, you MUST provide at least 3 distinct, human-like \`phrasings\`.
5.  **JSON Output:** The output MUST be a JSON object adhering to this exact structure: { "questions": [ { "question_id": "...", "context": {}, "parent_answer_id": null, "phrasings": ["..."], "answers": [ { "answer_id": "...", "answer_text": "...", "tags": ["..."] } ] } ] }
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!runState) {
            ...
            runState = {
                _id: RUN_ID, agent: AGENT_NAME, status: 'in_progress',
                categories_to_process: topLevelCategories.map(c => c.key),
                processed_categories: [], questions_in_progress: []
            };
        ...
        if (!nextCategoryKey) {
            ...
            await octokit.repos.createOrUpdateFileContents({
                owner: GITHUB_OWNER, repo: GITHUB_REPO, path: 'assets/questions.json',
                message: `feat(content): ${AGENT_NAME} - Weekly question architecture`,
                content, sha: fileSha, branch: 'main'
            });
        ...
        const aiResponse = await callAI({
            model: 'gpt-o3', prompt, agent_name: AGENT_NAME,
            estimated_cost: 25000, response_format: { type: "json_object" }
        });
